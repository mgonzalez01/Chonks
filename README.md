...
|---|---|---|
| AST-aware (full symbol and graph support) | `.c` (C) · `.cs` (C#) · `.cc .cpp .cu .cuh .cxx .h .hpp .hxx .inl .metal .mm` (C++) · `.gd` (GDScript) · `.cginc .compute .fx .fxh .hlsl .hlsli .raytrace` (HLSL) · `.cjs .js .jsx .mjs` (JavaScript) · `.lua` (Lua) · `.py .pyi` (Python) · `.tsx` (TSX) · `.ts` (TypeScript) | tree-sitter boundaries: functions, classes, methods. Named chunks that feed the symbol index, cross-reference graph, and repomap. |
