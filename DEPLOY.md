...
**An auto-discovered config with an absolute `codebase` is ignored.** `server.py` rejects an absolute `codebase` path from a config file it auto-discovered, that is, the cwd's `config.json`, `.chonks.json`, or `Chonks/config.json`, since a config dropped there could otherwise scope the `/index` endpoint to arbitrary locations. Passing `--config <path>` explicitly makes it an opt-in, and relative paths are always honoured, resolved against the cwd.
